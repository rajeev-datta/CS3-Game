...
void level_2_horizontals(vector_t top_right, double wall_length, double wall_height, scene_t *scene) {
    char *tank_info = malloc(sizeof(char *));
    tank_info = TANK_INFO;
    list_t *tank = animate_rectangle((vector_t) {100, TOP_RIGHT_COORD.y/2}, 50, 50);
    body_t *tank_body = body_init_with_info(tank, 50, color_get_red(), tank_info, free);
...
int main(int argc, char *argv[]) {
    ...
    *multi = false;
    ...
    while (!sdl_is_done(scene, scene_get_body(scene, 0), NULL, NULL, NULL, multi, NULL, NULL, NULL, NULL, NULL)) {
